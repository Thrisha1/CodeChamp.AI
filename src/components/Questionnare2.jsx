...
export default function Questionnare2({func, result}) {
    ...
    const nextBatch = Questionnaire.questions.slice(startIndex, startIndex + itemsPerPage);
    ...
    return (
        <div className="w-3/4 mx-10">
            <p className="text-3xl font-bold text-center p-5">Questionnaire : Basics of DSA</p>
            {nextBatch.map((q, index) => (
                <form key={index}>
                    <div
                        className={`my-5 bg-gray-200 p-10 w-full border-4 ${q && q?.correct === true ? "border-green-500" : "border-black"}`}>
                        <label className="text-xl flex mb-5 font-bold">{index + 1}. {q.question}</label>
                        ...
                        )}
                        {
                            resultArray.findIndex(r => r?.question === q.question) !== -1 ? (
                                <Result optionSelected={optionSelected}
                        ...
                        <div className="w-3/4 flex justify-between pt-10">
                            {
                                ...
                                resultArray.findIndex(r => r?.question === q.question) !== -1 ? (
                                    <>
                                        <button disabled type="submit" onClick={(e) => {
                                            e.preventDefault();
                                            handleSubmit();
            ...
            <div className="w-full flex justify-between">
                <button onClick={async (e) => {
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                         stroke="currentColor" className="w-6 h-6 group-hover:translate-x-2 duration-300">
    ...
    return (
        <>
            <div className={"text-xl border-2 border-green-500 w-max px-6 py-1 mt-5"}>
                {
                    correct?.correct === true ? <p className={"text-green-500"}>Correct</p> :
                        <p className={"text-red-500"}>Incorrect</p>
                }
            </div>
            {correct && (
                <Modals correct={correct} optionSelected={optionSelected}/>
            )}
        </>
    )
}
